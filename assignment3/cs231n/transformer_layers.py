...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[:, :, 0::2] = torch.sin(i * pows)
        pe[:, :, 1::2] = torch.cos(i * pows)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Y = Q @ K.transpose(2, 3) / math.sqrt(E / H) # (N, H, S, T)

        if attn_mask is not None:
          # Softmax에서 낮은 확률을 보장함
          Y.masked_fill(attn_mask == 0, float("-inf"))
        
        Y = self.attn_drop(F.softmax(Y, dim=-1)) @ V  # (N, H, S, E//H)
        output = self.proj(Y.moveaxis(1, 2).reshape(N, S, E)) # (N, S, E)
