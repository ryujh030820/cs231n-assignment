...
def rnn_step_backward(dnext_h, cache):
    ...
    dz = (1 - np.square(next_h)) * dnext_h # (N, H)

    dx = np.matmul(dz, Wx.T) # (N, D)
    dprev_h = np.matmul(dz, Wh.T) # (N, H)
    dWx = np.matmul(x.T, dz) # (D, H)
    dWh = np.matmul(prev_h.T, dz) # (H, H)
