...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l1 = -1 * torch.log(torch.exp(sim(z_k, z_k_N) / tau) / torch.exp(sims_k / tau).sum())
        l2 = -1 * torch.log(torch.exp(sim(z_k_N, z_k) / tau) / torch.exp(sims_k_N / tau).sum())
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(dim = 1, keepdim = True) / (torch.linalg.norm(out_left, dim = 1, keepdim = True) * torch.linalg.norm(out_right, dim = 1, keepdim = True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    loss = -1 * torch.log(numerator / denom).mean()
